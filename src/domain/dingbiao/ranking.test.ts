...
describe("dingbiao difference and winner ranking", () => {
  it("calculates the absolute difference between bidPrice and M", () => {
    expect(calculateDifferenceToBenchmark("123.45", "120.1")).toBe("3.35");
    expect(calculateDifferenceToBenchmark("116.75", "120.1")).toBe("3.35");
  });

  it("prefers the lower bid when differences are equal", () => {
  ...
  it("uses candidateId for stable ordering when difference and bid are equal", () => {
    const ranked = rankDingbiaoCandidates(
      [candidate("candidate-b", "100"), candidate("candidate-a", "100")],
      "100",
    );

    expect(ranked.map((result) => result.candidateId)).toEqual([
      "candidate-a",
      "candidate-b",
    ]);
  });
