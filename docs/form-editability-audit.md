...
8. 推优剔除复选在重复快速回调时可向数组重复追加 candidate ID：已改为幂等 Set 更新。
...
| 清标 | 4 组推优剔除单位 | Controlled Checkbox MultiSelect | 是 | 是，可取消/新增，规则级 dirty | 是 | Qingbiao input revision，后续定标由新清标来源推进 | Unit + Qingbiao verify + E2E 保存后重编 | PASS |
...
- Unit：项目类型任意增删、快速重复回调幂等；推优剔除增删和幂等。
...
- Playwright：从“幕墙+装修”开始，选中实验室、取消幕墙，保存后刷新断言“装修+实验室”；再次加入总包仍可保存。同一链路还验证了空选择失败后继续编辑、定标抽值二次修改、Candidate 二次编辑、履约 Select 二次编辑、推优剔除保存后取消/重选，以及仅修改 projectTypes 后清标 stale。
