...
> 2026-08-28 正式修正：本文关于“用户为规则1～4手工选择剔除单位”的内容仅保留为历史审计记录，已不再代表当前产品。当前规则是系统按全部候选投标总价自动生成4套剔除结果，详见 `docs/qingbiao-exclusion-rules.md`；当前 release baseline 为 Golden Case 20260828-B，20260820-A 保留为 legacy fixture。
...
系统先取得全部有效候选单位，使用 Decimal 按 `bidPrice DESC, candidateId ASC` 排序，然后自动生成4套剔除结果：规则1剔除最高价1家；规则2剔除最高价前2家；规则3剔除 `max(1, ROUND_HALF_UP(n/3))` 家；规则4剔除 `max(1, ROUND_HALF_UP(n/4))` 家。用户不能编辑 `excludedCandidateIds`。
...
例如 `10.38%` 的内部值是 `0.1038`，转为 `10.38` 个百分点后取整，再参与去重平均。候选数量舍入和 K1 整数百分点舍入均使用 `HALF_UP`；自动规则执行后没有剩余 K1 样本时返回结构化候选不足错误，不缩减剔除数量。

该自动剔除集合只用于生成 K1。后续报价与综合排名正式保持覆盖全部候选单位（`ALL_CANDIDATES`）。
...
2026-08-24 版本当时计划的中间流程为：

```text
推优规则 1..4 -> 用户选择剔除单位 -> 4个K1 -> K2 0/1/2/3
```

上述人工选择流程也已过时。当前 UI 展示4张只读自动规则 Card，候选报价变化后名单实时重派生，用户直接点击“清标测算”生成16场景。
